package com.knucapstone.tripjuvo.beacon;
...
public class RecoBackgroundMonitoringService extends Service implements RECOMonitoringListener, RECOServiceConnectListener {
    ...
    public void didEnterRegion(RECOBeaconRegion region, Collection<RECOBeacon> beacons) {
        ...
        Log.i("###", region.getMinor()+"");
        this.popupNotification("M : Inside of " + region.getUniqueIdentifier());
        //Write the code when the device is enter the region

        Intent intent = new Intent(getApplicationContext(), DialogUniversalInfoActivity.class);

        if(beacons.iterator().hasNext()) {
            intent.putExtra("asdf", beacons.iterator().next().getMinor());
        }
