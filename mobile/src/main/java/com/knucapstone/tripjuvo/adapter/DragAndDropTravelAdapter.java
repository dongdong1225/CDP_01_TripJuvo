...
public class DragAndDropTravelAdapter extends BaseAdapter implements Swappable {
	...
	private ArrayList<TravelDummyModel> mDummyModelList;

	public DragAndDropTravelAdapter(Context context, ArrayList<TravelDummyModel> dummyModelList) {
		mContext = context;
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			convertView = mInflater.inflate(R.layout.list_item_drag_and_drop_travel, parent, false);
			...
			holder.subtext = (TextView) convertView.findViewById(R.id.drag_and_drop_travel_subtext);
			holder.icon = (TextView) convertView.findViewById(R.id.drag_and_drop_travel_icon);
			holder.rating = (TextView) convertView.findViewById(R.id.drag_and_drop_travel_rating);
			convertView.setTag(holder);
		} else {
			holder = (ViewHolder) convertView.getTag();
		}

		TravelDummyModel dm = mDummyModelList.get(position);

		ImageUtil.displayImage(holder.image, dm.getImageURL(), null);
		holder.text.setText(dm.getText());
		holder.subtext.setText(dm.getSubText());
		holder.rating.setText(dm.getRating());
